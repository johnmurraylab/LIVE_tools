...
CD_In<-function(directory, prefix = "", suffix = ".csv",
  ...
  CD.term = paste0("^",prefix,".+",suffix,"$") #check file presence
  directory <- sub(x = directory, pattern = "[/\\\\]+$", replacement = "")
  files <- list.files(directory)
  CD.files <- grep(CD.term,files,value = F)
  if(!is.null(info.suffix)){ #TIME and Info files might be mistakenly grep'ed along with CD pattern, remove them
  ...
  CDDataL <- list()
  CDDataL[['CD']] <- list()
  #default info for all embryos
  embryos.Info <- lapply(embNames, function(emb) {
    data.frame(name = emb,depth_corrected = F,time_corrected = F,rotated = F)
  })
  names(embryos.Info) <- embNames
  #retrieve info
  if(!is.null(info.suffix)){
    info.path <- file.path(
      directory, paste0(info.prefix,embNames,info.suffix)
    )
    info_exists <- file.exists(info.path)
    # Warn if any info files are missing
    missing_info <- embNames[!info_exists]
    if(length(missing_info) > 0){
      warning(paste0(
        "Info files not found for the following embryos: ",
        paste(missing_info, collapse = ", "),
        ". Default info will be used."
      ))
    }
    # Load existing info files and merge with defaults
    for(emb in embNames[info_exists]){
      this.path <- file.path(
        directory, paste0(info.prefix,emb,info.suffix)
      )
      this_info <- read.csv(this.path, header = TRUE)
  ...
  for (emb in embNames) {
    ...
    if(!is.null(time.suffix)){
      time.file <- file.path(directory, paste0(time.prefix,emb,time.suffix))
    }
    else{time.file<-NULL}
    read_out <- readEmbryoTable(CD.Path, time.file)
    CDDataL[['CD']][[emb]] <- read_out[[1]]
    CDDataL[['info']][[emb]][, "time_corrected"] <-
      CDDataL[['info']][[emb]][, "time_corrected"] || read_out[[2]]
  }
...
#' CD_Save
#' @description
#' save cell tracking files along with the embryo information
#'
...
#' @param align_t decide the time of the `alignCell` at the end times of `alignCell` lives after alignment
#' \itemize{
#'  \item if the input is a numeric, the time will be moved so that all `alignCell` lives end at the input value as time
#'  \item if the input is `"NO"` the time column will not be modified
#'  \item if the input is `"mean"` (default) the align time will become the mean value of the end times of `alignCell` lives across all embryos
#' }
#' @param aligningCell the cell used to find common time point to align across embryos
